---
title: Next.js 컴파일러
description: Next.js 어플리케이션을 변환하고 경량화하는 Rust로 작성된 Next.js 컴파일러
---

Next.js 컴파일러는 [SWC](http://swc.rs/)를 사용하여 Rust로 작성되어 있으며, 자바스크립트 코드를 변환하고 경령화 해줍니다. 이를 통해 개별 파일의 변환 작업과 번들의 최소화 작업에 사용되는 Babel과 Terser의 역할을 대체합니다.

Next.js 컴파일러를 사용한 컴파일링 Babel보다 17배 빠르며, Next.js 12 버전부터 기본으로 활성화되었습니다. 기존의 Babel 구성이나 [지원되지 않는 기능](#unsupported-features)을 사용 중인 경우, Next.js는 자체 컴파일러를 비활성화하고 계속해서 Babel을 사용합니다.
...
- **확장성**: 라이브러리를 포크하거나 설계 제약 조건을 해결할 필요 없이 SWC는 Next.js 내부에서 Crate로 사용될 수 있습니다. //\* crate부분 확인 필요(https://doc.rust-lang.org/book/ch07-01-packages-and-crates.html)
...
JSX 속성을 삭제할 수 있습니다. 테스트를 자주 사용되며 `babel-plugin-react-remove-properties`과 비슷합니다.
...
`node_modules`이 아닌 어플리케이션에 있는 모든 `console.*` 호출을 지우며 `babel-plugin-transform-remove-console`과 비슷합니다.
...
Next.js의 swc 컴파일러는 v13 이후 기본적으로 경령화에 사용되며 Terser보다 7배 더 빠릅니다.
...
[handlebars](https://docs.rs/handlebars) 를 사용하여 'transform' 필드에서 대체 가져오기 경로를 템플릿으로 만듭니다. 다음 변수와 헬퍼 함수(helper functions)를 사용할 수 있습니다:
...
커스텀 Babel 설정을 사용하는 경우, [설정을 공유 부탁드립니다](https://github.com/vercel/next.js/discussions/30174). 향후 지원할 플러그인뿐 아니라 일반적으로 많이 사용되는 Babel을 최대한 이식하기 위헤 노력 중입니다.
