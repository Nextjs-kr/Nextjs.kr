...
`"use client"`은 서버 전용 코드와 클라이언트 코드 사이에 *위치*합니다. 파일 상단의 가져오기 위에 위치하여 서버 전용에서 클라이언트 부분으로 경계를 넘나드는 *중단점*을 정의합니다. 파일에 `"use client"`가 정의되면 하위 컴포넌트를 포함하여 파일로 가져온 다른 모든 모듈은 클라이언트 번들의 일부로 간주됩니다.
