...
라우팅 폴더와 파일 규칙 외에도, Next.js는 프로젝트 파일을 구조화하고 코로케이션시키는 방법에 대해 의견을 제시하지 않습니다.
...
`app` 디렉터리에서 [중첩된 폴더 계층 구조](/docs/app/building-your-application/routing#route-segments)가 라우트 구조를 정의합니다.
...
이는 `app` 디렉터리의 라우트 세그먼트 내에 실수로 라우팅되는 파일 없이 **프로젝트 파일**을 **안전하게 코로케이션 시킬 수** 있다는 것을 의미합니다.
...
> **참고**:
>
> - 이는 `pages` 디렉터리와는 다릅니다. `pages`의 모든 파일은 라우트로 간주됩니다.
> - `app` 디렉터리에 프로젝트 파일을 **공존시킬 수는 있지만** 그럴 **필요는 없습니다**. 원하는 경우 [`app` 디렉터리 외부에 유지할 수도 있습니다](#store-project-files-outside-of-app).
...
`app` 디렉터리의 파일은 [기본적으로 안전하게 코로케이션 되므로](#safe-colocation-by-default) 비공개 폴더는 코로케이션에 필요하지 않습니다. 그러나 다음과 같은 경우에 유용할 수 있습니다:

- UI 로직과 라우팅 로직을 분리하고 싶은 경우.
- 프로젝트 및 Next.js 생태계 전체에서 내부 파일을 일관되게 구조화하고 싶을 경우.
- 코드 편집기에서 파일을 정렬하고 그룹화하고 싶을 경우.
...
Next.js는 옵션으로 [`src` 디렉터리](/docs/app/building-your-application/configuring/src-directory) 내에 응용 프로그램 코드(`app` 디렉터리를 포함하여)를 저장할 수 있도록 지원합니다. 이를 통해 어플리케이션 코드를 프로젝트의 주요 설정 파일과 분리할 수 있습니다.

<Image
  alt="`src` 디렉터리를 포함하는 예시 폴더 구조"
...
### `app` 디렉터리 내부의 최상위 폴더에 프로젝트 파일 저장

이 전략은 모든 어플리케이션 코드를 **app 디렉토리의 루트**에 있는 공유 폴더에 저장합니다.

<Image
  alt="`app` 디렉터리 내부에 프로젝트 파일을 저장한 예시 폴더 구조"
