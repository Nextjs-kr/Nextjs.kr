---
title: 동적 라우트
description: 동적 라우트는 프로그래밍적인 방식으로 동적 데이터로부터 라우트 세그먼트를 생성하는데 사용될 수 있습니다.
related:
  title: 다음 단계
...
정확한 세그먼트 이름을 미리 알 수 없어 동적 데이터로 라우트를 생성하려는 경우, 요청 시 채워지거나 빌드 시 [미리 렌더링되는](#generating-static-params) 동적 세그먼트를 사용할 수 있습니다.

## Convention

동적 세그먼트는 폴더명을 대괄호로 감싸서 생성할 수 있습니다: `[폴더명]`. 예시로, `[id]` 또는 `[slug]`.

동적 세그먼트는 [`레이아웃`](/docs/app/api-reference/file-conventions/layout), [`페이지`](/docs/app/api-reference/file-conventions/page), [`라우트`](/docs/app/building-your-application/routing/router-handlers) 및 생성 메타데이터 함수에 `매개변수` 프로퍼티로 전달됩니다.
## Example
...
> **참고 사항**: 동적 세그먼트는 `pages` 디렉터리의 [동적 라우트](/docs/app/building-your-application/routing/dynamic-routes)와 동일합니다.
...
>**참고 사항**: 이 기능은 향후 [TypeScript plugin](/docs/app/building-your-application/configuring/typescript#typescript-plugin)에 의해 자동으로 처리될 수 있습니다.
