...
[Next.js의 Data Fetching](/docs/app/building-your-application/data-fetching/fetching)는 이제 데이터베이스 또는 콘텐츠 공급자 선택에 관해 규정하지 않고 가능한 한 end-to-end 타입 안전성을 제공합니다.
...
렌더링 되지 않은 데이터는 서버와 클라이언트 간에 전달되지 않으므로(서버에 남아 있음) 직렬화할 코드가 더 적습니다. 이는 현재 서버 컴포넌트 지원을 통해서만 가능합니다.
