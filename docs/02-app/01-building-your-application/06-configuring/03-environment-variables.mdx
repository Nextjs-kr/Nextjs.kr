...
- [`.env.local`을 사용하여 환경 변수를 로드합니다.](#환경-변수-로드하기)
- [`NEXT_PUBLIC_` 접두사를 사용하여 브라우저를 위한 환경 변수를 번들링합니다.](#브라우저를-위한-환경-변수-번들링하기)

## 환경 변수 로드하기

Next.js는 `.env.local`에서 환경 변수를 `process.env`로 로드하는 기능을 내장하고 있습니다.
...
이렇게 하면 `process.env.DB_HOST`, `process.env.DB_USER`, `process.env.DB_PASS`가 Node.js 환경에 자동으로 로드되어 라우트 핸들러에서 사용할 수 있습니다.
...
> **참고**: 앱이 빌드된 후에는 이러한 환경 변수의 변경에 더 이상 응답하지 않습니다. 예를 들어 Heroku 파이프라인을 사용하여 한 환경에서 빌드한 슬러그를 다른 환경으로 전환하거나, 단일 Docker 이미지를 여러 환경에 빌드 및 배포하는 경우, 모든 `NEXT_PUBLIC_` 변수는 빌드 시점에 평가된 값으로 고정되므로 이 값들은 프로젝트를 빌드할 때 적절하게 설정해야 합니다. 런타임 환경 값에 액세스해야 하는 경우, 자체 API를 설정하여 클라이언트에 제공해야 합니다 (요청 시 또는 초기화 중에).
...
> **참고**: `.env`, `.env.development`, `.env.production` 파일은 기본값을 정의하므로 레포지토리에 포함되어야 합니다. **`.env*.local` 은 `.gitignore`에 추가해야 합니다.** 해당 파일은 레포지토리에서 제외되어야 하기 때문입니다. `.env.local`에는 보안 정보를 저장할 수 있습니다.
...
`개발`과 `운영` 환경 외에도 세 번째 옵션인 `테스트` 환경이 있습니다. 개발 또는 운영 환경과 마찬가지로 `.env.test` 파일을 사용하여 `테스트` 환경에 대한 기본값을 설정할 수 있습니다 (앞의 두 가지만큼 일반적이지는 않습니다.). Next.js는 `테스트` 환경에서 `.env.development` 또는 `.env.production`의 환경 변수를 로드하지 않습니다.

이 기능은 `jest` 또는 `cypress`와 같은 도구로 테스트를 실행할 때 특정 테스트용 환경 변수를 설정해야 하는 경우 유용합니다. `NODE_ENV`를 `test`로 설정하면 테스트 환경에 대한 기본값이 로드되지만, 일반적으로 테스트 도구가 이를 자동으로 처리하므로 직접 설정할 필요는 없습니다.

`테스트` 환경과 `개발` 및 `운영` 환경 간에는 약간의 차이가 있으므로 이를 염두에 두어야 합니다. 테스트 환경에선 모든 사용자가 동일한 결과를 얻는 것을 기대하기 때문에 `.env.local`을 로드하지 않습니다. 이렇게 하면 각 테스트 실행은 `.env.local`(기본 설정을 오버라이드하기 위한 것)을 무시하고 여러 실행에서 동일한 환경 변수 기본값을 사용합니다.

> **참고**: 기본 환경 변수와 마찬가지로 `.env.test` 파일도 레포지토리에 포함해야 하지만, `.env*.local`은 `.gitignore`를 통해 무시하도록 되어있으므로 `.env.test.local`은 포함되어서는 안 됩니다.
