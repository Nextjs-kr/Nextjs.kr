...
> 다른 ESLint 설정과 함께 `eslint-config-next`를 사용하려면 [추가 설정]([#additional-config]) 섹션을 참조하여 충돌을 일으키지 않고 설정하는 방법을 알아보세요.
...
| <Check size={18} /> | [@next/next/no-img-element](/docs/messages/no-img-element)                                                               | LCP가 느려지고 대역폭이 높아지므로 `<이미지>` 요소의 사용을 방지합니다.                                         |
...
| <Check size={18} /> | @next/next/no-typos                                                                                                      | [Next.js의 데이터 불러오기 함수](/docs/pages/building-your-application/data-fetching) |에서 흔히 발생하는 오타를 방지합니다.
...
> [Create Next App](/docs/app/api-reference/create-next-app)로 빌드한 새 애플리케이션에는 `next/core-web-vitals` 엔트리 포인트가 자동으로 포함됩니다(/docs/app/api-reference/cate-next-app).

## 다른 도구와 함께 사용

### Prettier

또한 ESLint에는 코드 서식 지정 규칙이 포함되어 있어 기존 [Prettier](https://prettier.io/) 설정과 충돌할 수 있습니다. ESLint와 Prettier가 함께 작동하도록 하려면 ESLint 설정에 [eslint-config-prettier](https://github.com/prettier/eslint-config-prettier)를 포함할 것을 권장합니다.
...
이미 별도의 ESLint 설정을 사용하고 있고 `eslint-config-next`를 포함하려는 경우 다른 설정 다음에 마지막으로 확장해야 합니다. 예를 들어
