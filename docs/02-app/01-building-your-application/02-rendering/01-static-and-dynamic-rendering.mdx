...
- **정적** 라우트에서, 컴포넌트들은 빌드 타임에 서버에서 렌더링됩니다. 이 작업 결과는 잦은 요청에 대해 캐싱되고 재사용됩니다.
- **동적** 라우트에서, 컴포넌트들은 요청 시점에 서버에서 렌더링됩니다.
...
기본적으로 Next.js는 캐싱 동작이 명시적으로 제외되지 않은 `fetch()` 요청의 결과를 캐싱합니다. `cache` 옵션이 설정되지 않은 fetch 요청은 `force-cache` 옵션을 사용하게 된다는 것을 의미합니다.

만약 라우트 내에 있는 어떤 fetch 요청이 `revalidate` 옵션을 사용한다면, 해당 라우트는 재검증 중에 정적으로 리렌더링될 것입니다.

캐싱과 데이터 페칭 요청에 대해 더 알아보려면 [캐싱과 재검증](/docs/app/building-your-application/data-fetching/caching) 페이지를 참고하세요.

## Dynamic Rendering

정적 렌더링 과정에 동적 함수나 캐싱되지 않은 동적 `fetch()` 요청이 발경된다면 Next.js는 라우트 전체를 요청 시점에 동적으로 렌더링하도록 전환합니다. 캐싱된 데이터 요청은 동적 렌더링에서도 여전히 재사용될 수 있습니다.
...
데이터 페칭이 캐싱되든 아니든, 동적 함수는 항상 라우트를 동적 렌더링되도록 만든다는 점에 주목해야 합니다. 다시 말해, 정적 렌더링은 데이터 페칭 동작 뿐 아니라 라우트에서 사용하는 동적 함수에도 의존합니다.

> **알아두면 좋은 점**: Next.js는 미래에 라우트 전체가 아닌 독립적인 레이아웃과 페이지 단위로 정적 또는 동적으로 렌더링되는 하이브리드 서버 사이드 렌더링을 지원할 예정입니다.
...
- [`searchParams`](/docs/app/api-reference/file-conventions/page#searchparams-optional) [Pages](/docs/app/api-reference/file-conventions/page) prop을 사용하는 경우 페이지는 요청 시점에 동적 렌더링됩니다.
