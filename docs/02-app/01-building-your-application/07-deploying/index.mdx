...
- 클라이언트에서 리액트 반응성 주입을 위한 자바스크립트 파일
...
- `.next/server/pages` – Next.js 서버에서 프리 렌더링하기 위한 HTML과 자바스크립트의 진입점인 파일이 있습니다. [결과물 파일 추적](/docs/pages/api-reference/next-config-js/output) 기능을 활성화 했을 때, 페이지와 관련있는 모든 파일의 경로를 포함하고 있는 `.nft.json`으로 끝나는 파일을 생성합니다.
...
[Vercel](https://vercel.com?utm_source=next-site&utm_medium=docs&utm_campaign=next-website)은 설정 없이 Next.js를 가장 빠르게 배포할 수 있는 방법입니다.

Vercel으로 배포를 한다면, [Next.js를 자동으로 감지](https://vercel.com/solutions/nextjs?utm_source=next-site&utm_medium=docs&utm_campaign=next-website)하여 `next build` 명령을 실행하고, 빌드 결과물을 최적화합니다.

- 변경이 없다면 배포와 배포 사이의 정적파일을 지속적으로 캐시합니다.
- 모든 커밋은 중복없는 URL로 [영구으로 배포](https://vercel.com/features/previews?utm_source=next-site&utm_medium=docs&utm_campaign=next-website)가 됩니다.
...
도커나 Node.js를 사용하여 Next.js의 모든 기능을 직접 호스팅할 수 있습니다. [일부 제약을 있는](/docs/app/building-your-application/deploying/static-exports) 정적 HTML 추출도 가능합니다.

### Node.js 서버

Node.js를 지원하는 모든 호스팅 서비스를 통해 배포할 수 있습니다. e.g. [AWS EC2](https://aws.amazon.com/ec2/), [DigitalOcean Droplet](https://www.digitalocean.com/products/droplets/)
...
> **참고**: [위의 예제](#도커-이미지)에서 다룬 Dockerfile을 사용할 수 있는 관리형 플랫폼들 입니다.
...
Next.js 애플리케이션을 배포할 때, 리로드 하지 않고 최신 버전을 보고 싶어할 것 입니다.

Next.js는 페이지를 이동이 있을 때 백그라운드에서 애플리케이션의 최신버전을 자동으로 로드합니다. 클라이언트 사이드 이동인 경우 `next/link` 컴포넌트는 일시적으로 평범한 `<a>` 태그 처럼 동작합니다.

> **참고**: `next/link` 컴포넌트에 의해서 새로운 페이지(구 버전 포함)를 이미 prefetch 했다면, Next.js는 구버전을 사용합니다. prefetch _하지 않은_ (그리고 CDN에 캐시가 없는) 페이지로 이동한다면 최신 버전을 로드합니다.

## 직접 우아하게 종료하기

직접 호스팅 했을 때, `SIGTERM` 또는 `SIGINT` 시그널을 받아서 서버를 종료할 때 특정 코드를 실행하고 싶을 겁니다.
...
> **참고**: `next dev` 명령어에서는 직접 시그널을 처리하는 것이 불가능합니다.
