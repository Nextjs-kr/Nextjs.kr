...
[Server Actions](#server-actions) 는 React [Actions](#actions) 를 기반으로 구축된 Next.js의 **주요**기능 중 하나입니다. 이는 Client-Side의 Javascript 코드를 줄이고, Server-Side에서의 데이터 변경을 가능하게 하는 보다 발전된 형태의 기능입니다. 이는 Server Components 내부에서 정의되거나 Client Components에서 호출될 수 있습니다.

**Server Components에서 사용될 때:**
...
> **Good to know:**
>
> - Server Actions의 사용은 React의 `experimental` 채널에서 동작합니다.
> - React Actions, `useOptimistic` 그리고 `useFormStatus`은 Next.js나 React Server Components 만의 특별한 기능이 아닙니다.
> - Next.js는 React Actions의 기능을 `revalidateTag`나 `revalidatePath`과 같이 데이터 변경 API들을 추가하는 것을 포함하여 Next.js의 라우터, 번들러 그리고 캐싱 시스템까지 확장합니다.

## 컨벤션

당신은 당신의 Next.js 프로젝트의 **experimental** `serverActions` flagf를 활성화하여 Server Actions를 사용 가능하게 만들 수 있습니다.
...
Server Actions 는 두 곳에서 정의될 수 있습니다:

- 이것을 사용하는 컴포넌트 내부 (오직 서버 컴포넌트에서)
- 재사용성을 고려하여 각각 분리된 파일 (클라이언트, 서버 컴포넌트). 당신은 하나의 파일 내부에서 여러 개의 Server Actions를 정의할 수 있습니다.

#### 서버 컴포넌트에서

`"use server"` 키워드가 함수 최상단에서 명시된 비동기 함수 내부에서 Server Actions를 정의할 수 있습니다. 이 함수는 React Server Components 규약을 기반으로 한 **serializable arguments** 와 **serializable return value** 반드시 포함해야 합니다:
...
만일 당신이 Server Actions를 Client Components에서 사용하려 한다면, 당신의 action을 "use server"키워드가 함수 최상단에서 명시된 분리된 파일에서 작성해야 합니다. 그리고, 이것을 당신의 Client Components에 import하여 사용할 수 있습니다:
