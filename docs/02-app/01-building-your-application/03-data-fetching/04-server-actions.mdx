...
[Server Actions](#server-actions) 는 리액트의 [Actions](#actions) 를 기반으로 구축된 Next.js의 **주요**한 기능 중 하나입니다. 이는 클라이언트 단의 Javascript 코드를 줄이고, 서버 단에서의 데이터 변경을 가능하게 하는 보다 발전된 형태의 기능입니다. 이는 서버 컴포넌트 내부에서 정의되거나 클라이언트 컴포넌트에서 호출될 수 있습니다.
...
> **알아두면 좋은 것들**
>
> - Server Actions는 리액트의 `experimental` 채널에서 사용되고 동작합니다.
...
> **알아두면 좋은 것들**: `action`은 기본 HTML `form` 요소의 [`action`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/form#action)과 유사합니다.

#### `formAction`

`button`, `input type="submit"`, 그리고 `input type="image"`과 같은 요소에 존재하는 **Form Actions**을 다루기 위해 `formAction`을 사용할 수 있습니다. `formAction` 속성은 form의 `action` 보다 우선시됩니다.
...
> **알아두면 좋은 것들**: `formAction`은 HTML 원시 타입 [`formaction`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/button#formaction)입니다. 리액트는 이 속성에 함수를 전달할 수 있도록 해줍니다.
...
> **알아두면 좋은 것들**: `startTransition`을 사용하는 것은 즉시 사용 가능한 점진적 향상을 불가능하게 만든다.
...
Server Actions는 [불러오기](#불러오기)를 통해 사용되는 것이 권장되지만, 특정 상황에서 Server Actions를 클라이언트 컴포넌트의 인자로 받아와 사용해야 할 수 있습니다.
...
Server Actions는 [`form` 요소의`action` 속성](#불러오기)을 통해 점진적으로 향상될 수 있습니다. 이 form은 클라이언트 단의 Javascript가 로딩되기 전에 작동합니다. 이는 이 form의 제출에 리액트의 수화가 필요하지 않음을 뜻합니다.
