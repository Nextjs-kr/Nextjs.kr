...
Next.js 13으로 업그레이드하고 꼭 새로운 [App Router](/docs/app/building-your-application/routing#the-app-router)를 **사용하지 않아도 됩니다.** `pages` 디렉토리에서도 [Image component](#image-component), [Link component](#link-component), [Script component](#script-component), 그리고 [Font optimization](#font-optimization) 등 새로운 기능들을 두 디렉토리 모두에서 사용할 수 있습니다.
...
기존에 `_app` 또는 `_document` 파일이 존재하는 경우 콘텐츠(예시: global styles)를 루트 레이아웃(`app/layout.tsx`)에 복사할 수 있습니다. `app/layout.tsx`의 스타일은 `pages/*`에 _적용되지 않습니다._ 마이그레이션을 하는 동안 `_app`/`_document`를 유지하여 `pages/*` 경로가 손상되지 않도록 해야 합니다. 마이그레이션이 완료되면 안전하게 삭제할 수 있습니다.
...
`app` 디렉토리에서 [서버 컴포넌트](/docs/getting-started/react-essentials#server-components)를 사용하여 데이터 패칭을 React 컴포넌트 내부에 위치시킬 수 있습니다. 이 방법을 통해 서버에서 렌더링 된 HTML을 유지하면서 클라이언트에 더 적은 자바스크립트를 전송해도 됩니다.
