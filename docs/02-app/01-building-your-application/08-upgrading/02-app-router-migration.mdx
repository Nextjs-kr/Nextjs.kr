...
ESLint를 사용 중인 경우 ESLint 버전을 업그레이드해야 합니다.

```bash filename="Terminal"
npm install -D eslint-config-next@latest
```

> **알아두면 좋을 정보**: ESLint 변경 사항을 적용하기 위해 VS Code에서 ESLint 서버를 다시 시작해야 할 수 있습니다. 그 때 Command Palette (`cmd+shift+p` on Mac; `ctrl+shift+p` on Windows)를 열고 `ESLint: Restart ESLint Server`를 검색하세요.

## 다음 단계

업데이트를 했다면, 다음 섹션을 참조하여 다음 단계를 진행하세요.
...
Next.js 13에서는 새로운 기능과 규칙이 적용된 새로운 [App Router](/docs/app/building-your-application/routing)가 도입되었습니다. 새로운 라우터는 `app` 디렉토리에서 사용할 수 있으며 `pages` 디렉토리와 같이 존재합니다.

Next.js 13으로 업그레이드 하고 꼭 새로운 [App Router](/docs/app/building-your-application/routing#the-app-router)를 **사용하지 않아도 됩니다.** `pages` 디렉토리에서도 [Image component](#image-component), [Link component](#link-component), [Script component](#script-component), 그리고 [Font optimization](#font-optimization)등 새로운 기능들을 두 디렉토리 모두에서 사용할 수 있습니다.
...
이전에는 Next.js가 [inlining font CSS](/docs/app/building-your-application/optimizing/fonts)를 사용하여 글꼴을 최적하는 데 도움을 주었습니다. 버전 13에서는 [`next/font`](/docs/app/building-your-application/optimizing/fonts) 모듈이 도입되어 뛰어난 성능과 개인 정보를 보장하면서 글꼴 로드 환경을 사용자 지정할 수 있습니다. `next/font`는 `pages`와 `app` 디렉토리 모두에서 지원됩니다.

[inlining CSS](/docs/app/building-your-application/optimizing/fonts)는 여전히 `pages` 디렉토리에서는 작동하지만 `app` 디렉토리에서는 작동되지 않습니다. 그러므로 대신 [`next/font`](/docs/app/building-your-application/optimizing/fonts)를 사용해야 합니다.

[폰트 최적화](/docs/app/building-your-application/optimizing/fonts) 페이지에서 `next/font`를 사용하는 방법을 알아보세요.

## `pages`에서 `app`으로 마이그레이하기

App Router로 전환하면 서버 컴포넌트, Suspense 등과 같이 Next.js에서 빌드되는 React 기능을 처음으로 사용할 수 있습니다. [special files](/docs/app/building-your-application/routing#file-conventions)이나 [layouts](/docs/app/building-your-application/routing/pages-and-layouts#layouts)과 같은 새로운 Next.js 기능과 결합하면, 마이그레이션은 새로운 개념, 사고 모델 및 학습해야 할 행동 변화를 의미합니다.
...
- `pages/api/*`는 현재 `pages` 디렉토리 안에 남아있습니.
...
- 루트 레이아웃은`pages/_app.tsx`와 `pages/_document.tsx` 파일들을 대체합니다.
...
기존에 `_app` 또는 `_document` 파일이 존재하는 경우 콘텐츠(예시: global styles)을 루트 레이아웃(`app/layout.tsx`)에 복사할 수 있습니다. `app/layout.tsx`의 스타일은 `pages/*`에 _적용되지 않습니다._ 마이그레이션을 하는 동안 `_app`/`_document`를 유지하여 `pages/*` 경로가 손상되지 않도록 해야 합니다. 마이그레이션이 완료되면 안전하게 삭제할 수 있습니다.
...
- `DashboardLayout`의 내용을 [클라이언트 컴포넌트](/docs/getting-started/react-essentials#client-components)로 이동하여 `pages` 디렉토리의 동작을 유지합니다.
  ...
  'use client' // 이 명령어는 import 전에 파일의 맨 위에 있어야합니다.
...
- `DashboardLayout.js` (Client Component)의 상호작용하지 않는 부분을 `layout.js` (Server Component)로 점진적으로 전환하여 클라이언트에 전송하는 컴포넌트 자바스크립트의 양을 줄일 수 있습니다.
...
- `app`에서 [데이터 패칭](/docs/app/building-your-application/data-fetching)의 변화가 있습니다. `getServerSideProps`, `getStaticProps`와 `getInitialProps`가 더 간단한 API로 대체되었습니다.
...
- `pages/index.js`에서 데이터를 불러오는 경우, 새로운 [데이터 패칭 API](/docs/app/building-your-application/data-fetching/fetching)를 사용하여 데이터 패칭 로직을 서버 컴포넌트로 직접 이동하세요. 자세한 내용은 [데이터 패칭 업그레이드 가이드](#step-6-migrating-data-fetching-methods)를 참조하세요.
...
- `useSearchParams`과 `usePathname`를 함께 사용하여 페이지 변경 사항을 확인할 수 있습니다. 자세한 내용은 [라우터 이벤트](/docs/app/api-reference/functions/use-router#router-events) 섹션을 참조하세요.
...
- `isReady`는 더 이상 필요하지 않아 제거되었습니. [정적 렌더링](/docs/app/building-your-application/rendering#static-rendering) 과정에서 [`useSearchParams()`](/docs/app/api-reference/functions/use-params) hook을 사용하면 모든 컴포넌트는 사전 렌더링 단계를 생략하고 대신 런타임 때 클라이언트에서 렌더링됩니다.

[`useRouter()` API 레퍼런스 확인하기](/docs/app/api-reference/functions/use-router).

### 6단계: 데이터 패칭 메서드 마이그레이션하
...
```tsx filename="app/page.tsx" switcher
export default async function Page() {
  // 해당 요청은 수동으로 무효화될 때까지 캐싱되어야 합니다.
  ...
  // 해당 요청은 10초의 유지 시간을 가지며 캐싱되어야 합니다.
...
`pages` 디렉토리에서 `getServerSideProps`는 서버에서 데이터를 가져오고 파일에 있는 내보내는 React 컴포넌트로 props를 전달하기 위해 사용합니다. 페이지 초기 HTML은 서버에서 미리 렌더링된 후 브라우저의 페이지에서 "hydrating"(인터랙티브하게 만들기)됩니다.
...
`app` 디렉토리에서 [서버 컴포넌트](/docs/getting-started/react-essentials#server-components)를 사용하여 데이터 패칭을 React 컴포넌트 내부에 위치시킬 수 있습니다. 이 방법을 통해 서버에서 렌더링된 HTML을 유지하면서 클라이언트에 더 적은 자바스크립트를 전송해도 됩니다.
...
export default async function Page() {
  ...
  // 직접 사용하거나 데이터 패칭 함수에서 사용할 수 있습니다.
...
`app` 디렉토리에서 [`fetch()`](/docs/app/api-reference/functions/fetch)를 통해 데이터 패칭을 하면 기본적으로 `cache: 'force-cache'`가 설정되어 있어 수동으로 이를 해제할 때까지 요청 데이터를 캐시합니다. 이는 `pages` 디렉토리의 `getStaticProps`와 유사하게 동작합니다.
...
`app` 디렉토리에서 [`config.dynamicParams` property](/docs/app/api-reference/file-conventions/route-segment-config#dynamicparams)은 [`generateStaticParams`](/docs/app/api-reference/functions/generate-static-params) 이외의 매개변수가 처리되는 방식을 제어합니다.
...
[`dynamicParams`](/docs/app/api-reference/file-conventions/route-segment-config#dynamicparams)를 `true` (기본값)로 설정할 때, 생성되지 않은 경로 세그먼트가 요청되면 서버에서 렌더되고 성공적으로 [정적 데이터](/docs/app/building-your-application/data-fetching#static-and-dynamic-data-fetching)로 캐싱됩니다.
...
`app` 디렉토리에서 [`fetch()`](/docs/app/api-reference/functions/fetch)로 데이터 패칭을 하는 경우 `revalidate`를 사용하여 지정된 시간(초) 동안 요청을 캐싱할 수 있습니다.
...
라우트 핸들러를 사용하면 [요청](https://developer.mozilla.org/en-US/docs/Web/API/Request) 및 [응답](https://developer.mozilla.org/en-US/docs/Web/API/Response) API를 사용하여 지정된 라우트에 대한 사용자 설정 요청 핸들러를 만들 수 있습니다.
...
> **참고**: 이전에 API 라우트를 사용하여 클라이언트에서 외부 API를 호출했다면, 이제 [서버 컴포넌트](/docs/getting-started/react-essentials#server-components)를 사용하여 데이터를 안전하게 가져올 수 있습니다. 자세히 알아보려면 [데이터 패칭](/docs/app/building-your-application/data-fetching/fetching)을 확인하세요.
