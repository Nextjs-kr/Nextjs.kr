...
스크립트 구성 요소에서 사용하지 않는 `<script>` 요소에 할당할 수 있는 DOM 속성은 [`nonce`](https://developer.mozilla.org/en-US/docs/Web/HTML/Global_attributes/nonce), [custom data attributes](https://developer.mozilla.org/en-US/docs/Web/HTML/Global_attributes/data-*) 등 다수 존재합니다. 추가 속성을 포함하면 HTML에 포함된 최종 최적화된 `<script>`요소로 자동 전달됩니다.
