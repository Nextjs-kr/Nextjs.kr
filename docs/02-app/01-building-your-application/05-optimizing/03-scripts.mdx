...
각 strategy 전략과 사용 사례에 대한 자세한 내용은 [`next/script`](/docs/app/api-reference/components/script#strategy) 문서를 참조하세요.

### Web Worker에 스크립트 오프로딩하기 (실험적 기능)

> **경고**: `worker` 전략은 아직 안정화되지 않았으며 [`app`](/docs/app/building-your-application/routing/defining-routes) 디렉터리에서 작동하지 않습니다. 주의해서 사용하세요.
