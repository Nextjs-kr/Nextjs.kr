...
> - 접미사를 추가하기 위해 [`pagesExtension` 설정 옵션](/docs/pages/api-reference/next-config-js/pageExtensions)을 사용한다면 `instrumentation` 파일이름 또한 일치시키기 위해 업데이트해야 합니다.
...
경우에 따라 특정 파일이 가지고 있는 사이드 이펙트를 위해 해당 파일을 코드에 포함하는 것이 유용할 수 있습니다. 예를 들어 전역 변수가 정의된 파일을 명시적으로 사용하지는 않지만 가져올 수 있습니다. 이를 통해 해당 패키지가 선언한 전역 변수에 접근할 수 있습니다.
...
그러나 이보단 사이드 이펙트를 가지는 파일을 불러올 때는 register 함수 내에서 `import` 하는 것을 권장합니다. 다음은 `register` 함수에서 `import` 문을 사용하는 기본 예제입니다.
