...
`register` 함수가 배포되면, 이는 각각의 cold boot마다 호출됩니다. (각 환경마다 정확히 한 번씩 호출됩니다.)
...
그러나 우리는 부수 효과를 가지는 파일을 불러올 때 register 함수 내에서 `import` 하는 것을 권장합니다. 다음은 `register` 함수에서 `import` 문을 사용하는 기본 예제입니다.
