...
경우에 따라 특정 파일이 가지고 있는 사이드 이펙트를 피하기 위해 해당 파일을 코드에 불러오는 것이 유용할 수 있습니다. 예를 들어 전역 변수가 정의된 파일을 명시적으로 사용하지는 않지만 가져올 수 있습니다. 이를 통해 해당 패키지가 선언한 전역 변수에 접근할 수 있습니다.
