...
description: 애플리케이션의 로딩 성능 개선을 위한 라이브러리와 React 컴포넌트의 지연된 가져오기.
---
...
이를 통해 **클라이언트 컴포넌트**와 불러와진 라이브러리의 로딩을 연기할 수 있고, 오직 클라이언트 번들에서 그것들이 필요할 때만 포함하도록 할 수 있습니다. 예를 들어, 사용자가 클릭해서 열기 전까지는 모델의 로딩을 지연하고 싶다고 해봅시다.
...
서버 컴포넌트를 동적으로 가져온다면, 서버 컴포넌트의 자식인 클라이언트 컴포넌트만이 지연 로드될 것입니다 - 서버 컴포넌트 자체가 아니라.
...
클라이언트 사이드에서 컴포넌트를 동적으로 로드하기 위해, 서버 렌더링을 사용하지 못하도록 `ssr` 옵션을 사용할 수 있습니다. 만약 외부 의존성 또는 컴포넌트가 `window` 같은 브라우저 API에 의존한다면 유용할 것입니다.
