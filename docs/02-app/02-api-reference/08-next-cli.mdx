...
`next build`는 애플리케이션의 최적화된 프로덕션 빌드를 생성합니다. 각 경로에 대한 정보가 출력됩니다.
...
이 플래그를 사용하면 다시 쓰기, 리디렉션 및 헤더와 같은 추가 빌드 출력이 표시됩니다.
...
Next.js를 다운스트림 프록시 (e.g. AWS ELB/ALB 같은 로드 밸런서) 뒤에 배포할 때, Next의 기본 HTTP 서버의 [keep-alive 타임아웃](https://nodejs.org/api/http.html#http_server_keepalivetimeout)를 다운스트림 프록시의 타임아웃보다 크게 설정하는 것이 중요합니다. 그렇지 않으면, 주어진 TCP 연결에 대해 keep-alive 타임아웃이 도달할 때, Node.js는 다운스트림 프록시에 알림 없이 즉시 연결을 종료합니다. 프록시가 Node.js에서 이미 종료한 연결을 재사용하려 시도할 때마다 프록시 에러가 발생합니다.

프로덕션 Next.js 서버의 타임아웃 값을 설정하려면, `next start`에 `-keepAliveTimeout` (밀리초 단위)를 전달하면 됩니다.
...
다른 디렉토리에서 린트를 하고 싶다면, `--dir` 플래그를 사용해 지정할 수 있습니다.
...
Next.js는 일반적인 사용에 대해 **완전히 익명*** 텔레메트리 데이터를 수집합니다.
...
`next info`는 현재 시스템에 대한 관련 세부 정보를 출력하며, Next.js 버그를 보고하는 데 사용될 수 있습니다.
이 정보는 운영체제 플랫폼/아키텍처/버전, 이진 파일 (Node.js, npm, Yarn, pnpm) 및 패키지 버전(`next`, `react`, `react-dom`)이 포함됩니다.
