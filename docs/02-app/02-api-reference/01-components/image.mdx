...
`blur`인 경우 `blurDataURL` 프로퍼티는 플레이스홀더로 사용됩니다. `src`가 [정적으로 가져온](/docs/app/building-your-application/optimizing/images#local-images) 이미지이고 `.jpg`, `.png`, `.webp`, 또는 `.avif`인 경우 `blurDataURL`은 자동으로 채워집니다.

동적 이미지의 경우 `blurDataURL` 프로퍼티를 제공해야 합니다. [Plaiceholder](https://github.com/joe-bell/plaiceholder) 와 같은 솔루션은 `base64` 생성에 도움이 될 수 있습니다.
...
`next.config.js` 파일에서 `images.imageSizes` 프로퍼티를 사용하면 이미지 너비 목록을 지정할 수 있습니다. 너비는 [디바이스 크기](#devicesizes) 배열과 연결되어 이미지 [소스셋](https://developer.mozilla.org/en-US/docs/Web/API/HTMLImageElement/srcset)을 생성하는데 사용되는 전체 크기 배열을 형성합니다.
