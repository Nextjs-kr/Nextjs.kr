...
> **참고**: Next.js 13 버전부터 컴포넌트의 이름이 변경되었으므로 [next/legacy/image](/docs/pages/api-reference/components/image-legacy) 문서를 보는 것이 좋습니다.

</PagesOnly>

이 API 레퍼런스는 Image 컴포넌트에서 사용할 수 있는 [props](#props)와 [구성 옵션](#configuration-options)의 사용법을 이해하는 데 도움이 됩니다. 기능 및 사용법은 이미지 [컴포넌트 페이지](/docs/app/building-your-application/optimizing/images)를 참조하세요.
...
`width` 속성은 렌더링 되는 너비를 픽셀 단위로 나타내므로 이미지가 표시되는 크기에 영향을 줍니다.

[정적으로 가져온 이미지](/docs/app/building-your-application/optimizing/images#local-images)나 [`fill` 속성](#fill)이 있는 이미지를 제외하고는, 필수입니다.

### `height`

`height` 속성은 렌더링 되는 높이를 픽셀 단위로 나타내므로 이미지가 표시되는 크기에 영향을 줍니다.

[정적으로 가져온 이미지](/docs/app/building-your-application/optimizing/images#local-images)나 [`fill` 속성](#fill)이 있는 이미지를 제외하고는, 필수입니다.

### `alt`

`alt` 속성은 스크린 리더와 검색 엔진에 이미지를 설명하는 데 사용됩니다. 또한 이미지가 비활성화되었거나 이미지를 로드하는 동안 오류가 발생한 경우에 대체하여 보여줄 텍스트이기도 합니다.
...
이미지가 단순한 장식용이거나 사용자를 위한 것이 아닌 경우 `alt` 속성은 빈 문자열(`alt=""`)이어야 합니다.

[자세히 알아보기](https://html.spec.whatwg.org/multipage/images.html#alt)

## 선택 Props

`<Image />` 컴포넌트는 필수 속성 외에도 여러 가지 추가 속성을 사용할 수 있습니다. 이 섹션에서는 Image 컴포넌트에서 일반적으로 사용되는 속성에 관해 설명합니다. 자주 사용되지 않는 속성에 대한 자세한 내용은 [고급 Props](#advanced-props) 섹션에서 확인할 수 있습니다.
...
- 첫째, `sizes` 값은 브라우저가 `next/image`의 자동 생성된 소스 세트에서 다운로드할 이미지의 크기를 결정하는 데 사용됩니다. 브라우저는 이미지를 선택할 때 아직 페이지에 있는 이미지의 크기를 알지 못하므로 뷰포트와 같은 크기이거나 더 큰 이미지를 선택합니다. `sizes` 속성을 사용하여 브라우저에 이미지가 실제 전체 화면보다 작을 것이라고 알릴 수 있습니다. `fill` 속성이 있는 이미지에 `sizes` 값을 지정하지 않으면 기본값인 `100vw` (전체 화면 너비)가 사용됩니다.

- 둘째, `sizes` 속성은 `next/image`가 이미지 소스 집합을 자동으로 생성하는 방법을 구성합니다. `sizes` 값이 없으면 고정된 크기를 갖는 이미지에 적합한 작은 소스 세트가 생성됩니다. `sizes`가 정의되어 있으면 반응형 이미지에 적합한 큰 소스 세트가 생성됩니다. `sizes` 속성에 뷰포트 너비의 백분율을 나타내는 `50vw`와 같은 크기가 포함된 경우, 소스 세트는 너무 작아서 필요하지 않은 값을 포함하지 않도록 잘립니다.

예를 들어 스타일링으로 인해 이미지가 모바일 디바이스에서는 전체 너비, 태블릿에서는 2열 레이아웃, 데스크탑 디스플레이에서는 3열 레이아웃으로 표시되는 것을 알고 있다면 다음과 같은 `sizes` 속성을 포함해야 합니다.
...
[Largest Contentful Paint (LCP)](https://nextjs.org/learn/seo/web-performance/lcp) 요소로 감지된 모든 이미지에는 `priority` 속성을 사용해야 합니다. 뷰포트 크기에 따라 이미지가 LCP 요소가 될 수 있으므로 여러 개의 우선순위 이미지를 사용하는 것이 적절할 수 있습니다.
...
`blur`인 경우 `blurDataURL` 속성이 플레이스홀더로 사용됩니다. `src`가 [정적으로 가져온](/docs/app/building-your-application/optimizing/images#local-images) 이미지이고 `.jpg`, `.png`, `.webp`, 또는 `.avif`인 경우 `blurDataURL`은 자동으로 채워집니다.

동적 이미지의 경우 `blurDataURL` 속성을 제공해야 합니다. [Plaiceholder](https://github.com/joe-bell/plaiceholder) 와 같은 솔루션은 `base64` 생성에 도움이 될 수 있습니다.
...
> **추천**: 이 속성은 고급 사용 사례에만 적용됩니다. 이미지를 `eager` 로드하도록 전환하면 일반적으로 **성능이 저하**됩니다. 대신 이미지를 미리 로드하는 [`priority`](#priority) 속성을 사용하는 것이 좋습니다.
...
악의적인 사용자로부터 애플리케이션을 보호하기 위해서 외부 이미지를 사용할 때는 별도 구성이 필요합니다. 이렇게 허용된 외부 이미지만 Next.js 이미지 최적화 API를 통해 제공될 수 있습니다. 이러한 외부 이미지는 아래와 같이 `next.config.js` 파일에서 `remotePatterns` 속성을 사용하여 구성할 수 있습니다.
...
> **참고**: 위의 예에서는 `next/image`의 `src` 속성이 `https://example.com/account123/`으로 시작해야 합니다. 다른 프로토콜, 호스트 이름, 포트 또는 일치하지 않는 경로가 있으면 400 Bad Request로 응답합니다.

다음은 `next.config.js` 파일에 있는 `remotePatterns` 속성의 또 다른 예입니다.
...
> **참고**: 위의 예에서는 `next/image`의 `src` 속성이 `https://img1.example.com` 또는 `https://me.avatar.example.com` 또는 하위 도메인 수와 관계 없이 시작해야 합니다. 다른 프토콜이나 일치하지 않는 호스트 이름은 400 Bad Request로 응답합니다.
...
다음은 `next.config.js` 파일에 있는 `domians` 속성의 예시입니다.
...
> **참고**: Customizing the image loader file, which accepts a function, require using [Client Components](/docs/getting-started/react-essentials#client-components) to serialize the provided function.

함수를 전달받는 이미지 로더 파일을 커스텀하는 경우 제공된 함수를 직렬화하기 위해 [클라이언트 컴포넌트](/docs/getting-started/react-essentials#client-components)를 사용해야 합니다.
...
사용자의 기기 너비를 알고 있는 경우 `next.config.js`의 `deviceSizes` 속성을 사용하여 기기 너비 중단점 목록을 지정할 수 있습니다. 이 너비는 `next/image` 컴포넌트가 `sizes` 속성을 사용할 때 사용자 기기에 맞는 이미지가 표시되도록 하기 위해 사용됩니다.
...
`next.config.js` 파일에서 `images.imageSizes` 속성을 사용하면 이미지 너비 목록을 지정할 수 있습니다. 너비는 [디바이스 크기](#devicesizes) 배열과 연결되어 이미지 [소스셋](https://developer.mozilla.org/en-US/docs/Web/API/HTMLImageElement/srcset)을 생성하는데 사용되는 전체 크기 배열을 형성합니다.
...
`Accept` 헤드가 구성된 포맷 중 둘 이상과 일치하는 경우 첫 번째로 일치하는 항목이 사용됩니다. 따라서 배열 순서가 중요합니다. 일치하는 항목이 없거나 소스 이미지가 [애니메이션](#animated-images)인 경우 최적화 API는 원본 이미지의 형식으로 폴백합니다.
...
캐싱과 최적화된 이미지의 TTL(Time to Live)을 초 단위로 구성할 수 있습니다. 대부분의 경우 파일 내용을 자동으로 해시하고 `Cache-Control` 헤더를 `불변`으로 설정하여 이미지를 영구적으로 캐시 하는 정적 이미지 가져오기를 사용하는 것이 좋습니다.
...
최적화된 이미지의 만료(또는 MaxAge)는 `minimumCacheTTL` 또는 업스트림 이미지 `Cache-Control` 헤더 중 더 큰 값으로 정의됩니다.

이미지별로 캐싱 동작을 변경해야 하는 경우, 업스트림 이미지(예: `/_next/image` 자체가 아닌 `/some-asset.jpg`)에 [`헤더`](/docs/app/api-reference/next-config-js/headers)를 구성하여 `Cache-Control` 헤더를 설정할 수 있습니다.

현재로서는 캐시를 무효화하는 메커니즘이 없으므로 `minimumCacheTTL`을 낮게 유지하는 것이 가장 좋습니다. 그렇지 않으면 `src` 프로퍼티를 수동으로 변경하거나 `<distDir>/cache/images`를 삭제해야 할 수 있습니다.

### `disableStaticImages`

기본 동작은 `'./icon.png'에서 아이콘 가져오기`와 같은 정적 파일을 가져온 다음 이를 `src` 속성으로 전달할 수 있습니다.
...
| `v13.0.0` | `next/image` 가져오기 이름이 `next/legacy/image`로 변경되었습니다. `next/future/image` 가져오기 이름이 `next/image`로 변경되었습니다. codemod를 사용하면 가져오기 이름을 안전하게 자동으로 변경할 수 있습니다. `<span>` 래퍼가 제거되었습니다. `layout`, `objectFit`, `objectPosition`, `lazyBoundary`, `lazyRoot` prop이 제거되었습니다. `alt`는 필수입니다. `onLoadingComplete`는 `img` 요소에 대한 참조를 받습니다. 내장 로더 구성이 제거되었습니다. |
