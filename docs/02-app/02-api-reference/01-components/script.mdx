...
다음은 스크립트 컴폰너트에 사용할 수 있는 Props 요약입니다.
...
`afterInteractive` 전략을 사용하는 스크립트는 클라이언트 측에서 HTML에 삽입되며 페이지에서 일부(또는 모든) 하이드레이션이 발생한 후 로드됩니다. **이 전략은 Script 컴포넌트의 기본 전략**이며, 가능한 한 빨리 로드해야 하지만 서드파티 Next.js 코드 이전에는 로드되지 않는 모든 스크립트에 사용해야 합니다.
...
일부 서드파티 스크립트에서는 스크립트 로드가 완료된 후 컴포넌트가 마운트될 때마다(예: 경로 탐색 후) 사용자가 JavaScript 코드를 실행해야 합니다. 스크립트가 처음 로드될 때 스크립트 로드 이벤트 후에 코드를 실행한 다음 onReady 프로퍼티을 사용하여 모든 후속 컴포넌트를 다시 마운트할 수 있습니다.
