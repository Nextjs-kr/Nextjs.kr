...
현재 Turbopack은 webpack 로더 API의 일부분을 지원하기 때문에 일부 webpack 로더를 사용하여 Turbopack 코드를 변환할 수 있습니다.
...
Turbopack은 `next.config.js`를 통해 webpack의 [`resolve.alias`](https://webpack.js.org/configuration/resolve/#resolvealias) 구성과 유사하게 모듈 분석 처리를 별칭으로 수정하도록 구성할 수 있습니다.
...
webpack에서 Turbopack으로 앱을 마이그레이션하는 방법에 대한 자세한 내용과 지침은 [webpack 호환성에 대한 Turbopack 설명서](https://turbo.build/pack/docs/migrating-from-webpack)를 참조하세요.
