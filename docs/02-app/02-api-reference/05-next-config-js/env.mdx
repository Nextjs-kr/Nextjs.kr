...
Next.js는 빌드 시 `process.env.customKey`를 `'my-value'`로 바꿉니다. 웹팩에서 제공하는 [DefinePlugin](https://webpack.js.org/plugins/define-plugin/)으로 인해 `process.env` 변수는 구조분해되지 않습니다.
