---
title: 출력
description: 애플리케이션을 편리하게 배포할 수 있도록 Next.js는 각 페이지마다 어떤 파일이 필요한지를 자동으로 추적합니다. 이 문서를 통해 동작 방식을 알아봅시다.
---

빌드 과정에서 Next.js는 각 페이지와 해당 페이지의 의존성을 자동으로 추적하여 프로덕션 버전의 애플리케이션을 배포하는데 필요한 모든 파일을 결정합니다.
...
추가로 `next start` 대신에 사용할 수 있는 최소한의 `server.js` 파일도 출력합니다. `public`과 `.next/static` 폴더는 각각 `server.js` 파일이 자동으로 제공하는 `standalone/public`과 `standalone/.next/static` 폴더로 복사됩니다. 하지만 이 최소한의 서버는 이상적으로는 CDN이 대신 처리해야하는 `public` 또는 `.next/static` 폴더를 기본값으로 복사하지 않습니다.

> **참고**:
>
> - `next.config.js`는 `next build` 과정 중에 읽혀서 `server.js` 출력 파일로 직렬화됩니다. 이전 버전의 [`serverRuntimeConfig` 또는 `publicRuntimeConfig` 옵션](/docs/pages/api-reference/next-config-js/runtime-configuration)이 사용된 경우에 해당 값은 빌드 시점의 값에 따라 구체화됩니다.
