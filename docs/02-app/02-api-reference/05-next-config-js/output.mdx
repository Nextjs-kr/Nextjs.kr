---
title: 출력
description: 어플리케이션을 편리하게 배포할 수 있도록 Next.js는 각 페이지마다 어떤 파일이 필요한지를 자동으로 추적합니다. 이 문서를 통해 동작 방식을 알아봅시다.
---

빌드 과정에서 Next.js는 각 페이지와 해당 페이지의 의존성을 자동으로 추적하여 프로덕션 버전의 어플리케이션을 배포하는데 필요한 모든 파일을 결정합니다.
