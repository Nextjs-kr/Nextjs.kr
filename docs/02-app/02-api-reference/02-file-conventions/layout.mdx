...
**root layout** 은 `app` 디렉토리 최상단의 **layout**을 의미합니다. `<html>`과 `<body>` 태그를 포함한 프로젝트 전역에서 공유하는 UI를 정의하는 데 쓰입니다.
...
[Pages](/docs/app/api-reference/file-conventions/page)와 다르게, 레이아웃 컴포넌트는 `searchParams` prop 을 **받아 오지 않습니다**. 이는 공유되는 레이아웃은 [페이지 이동 중 다시 렌더링 되지 않아](/docs/app/building-your-application/routing#partial-rendering) 이전 페이지의 `searchParams`를 가져오는 경우가 발생할 수 있기 때문입니다.
...
- 다수의 루트 레이아웃을 만들기 위해선 [route groups](/docs/app/building-your-application/routing/route-groups) 를 사용할 수 있습니다.
  - **서로 다른 루트 레이아웃 사이를** 이동하는 것은 페이지를 **완전히 새로고침하도록** 합니다. (클라이언트측 이동과는 반대로). 예를 들어, `app/(shop)/layout.js` 를 사용하는 `/cart` 에서 `app/(marketing)/layout.js` 를 사용하는 `/blog` 로 이동하는 것은 페이지를 완전히 새로고침 하도록 할 것입니다. **다수의 루트 레이아웃을 사용할 때에만** 이런 현상이 발생합니다.
