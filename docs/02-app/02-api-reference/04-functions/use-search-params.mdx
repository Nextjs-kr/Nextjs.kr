...
이렇게 하면 페이지의 일부가 정적으로 렌더링 되는 반면, `searchParams`를 사용하는 동적인 부분은 클라이언트 측에서 렌더링 됩니다.
...
페이지와 달리 [레이아웃](/docs/app/api-reference/file-conventions/layout)(서버 컴포넌트)은 searchParams 프로퍼티를 **받지 않습니다.** 이는 탐색 중에 공유 레이아웃이 [탐색 중에는 다시 렌더링 되지 않아](/docs/app/building-your-application/routing#partial-rendering) `searchParams`이 오래될 수 있기 때문입니다. [자세한 설명](/docs/app/api-reference/file-conventions/layout#layouts-do-not-receive-searchparams)을 확인하세요.

대신 클라이언트 컴포넌트에서 페이지 [`searchParams`](/docs/app/api-reference/file-conventions/page) 프로퍼티 또는 [`useSearchParams`](/docs/app/api-reference/functions/use-params) hook을 사용하면 클라이언트에서 최신 `searchParams`를 사용하여 다시 렌더링 됩니다.
