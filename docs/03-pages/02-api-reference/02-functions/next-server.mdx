...
- `cookies` - 쿠키와 함께 `Request`에 포함되어있는 [RequestCookies](https://edge-runtime.vercel.app/packages/cookies#for-request) 인스턴스입니다. 요청의 `Cookie` 헤더를 읽거나 수정합니다. [미들웨어에서 쿠키 사용하기](/docs/app/building-your-application/routing/middleware#using-cookies)를 참고하세요.
  ...
  - `set` - [W3C CookieStore API](https://wicg.github.io/cookie-store/#dictdef-cookielistitem) 사양에서 정의된 `CookieListItem` 프로퍼티로 이루어진 객체를 받는 메소드입니다.
...
- `nextUrl`: 확장되고 파싱된 URL 객체를 포함하고 있습니다. 이 객체는 `pathname`, `basePath`, `trailingSlash` 그리고 `i18n` 등과 같은 Next.js의 특정 프로퍼티를 포함하고 있으며 목록은 다음과 같습니다:
  - `basePath` (`string`)
...
- `NextResponse` 클래스에서는 public 메소드를 사용할 수 있습니다. 사용하는 방식에 따라서, 인스턴스를 생성하고 변수에 할당한 후 다음과 같은 public 메소드에 접속할 수 있습니다:
