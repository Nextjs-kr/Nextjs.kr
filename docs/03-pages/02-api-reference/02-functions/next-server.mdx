...
`NextRequest` 객체는 네이티브 [`Request`](https://developer.mozilla.org/en-US/docs/Web/API/Request) 인터페이스의 확장된 버전이며, 다음과 같은 메소드와 프로퍼티가 추가되었습니다.

- `cookies` - 쿠키와 함께 `Request`에 포함되어있는 [RequestCookies](https://edge-runtime.vercel.app/packages/cookies#for-request) 인스턴스입니다.
  ...
  - `set` - [W3C CookieStore API](https://wicg.github.io/cookie-store/#dictdef-cookielistitem) 사양에서 정의된 `CookieListItem` 이루어진 객체를 받는 메소드입니다.
...
- `nextUrl`: 확장되고 파싱된 URL 객체를 포함하고 있습니다. 이 객체는 `pathname`, `basePath`, `trailingSlash` 그리고 `i18n` 등과 같은 Next.js의 프로퍼티를 포함하고 있으며 목록은 다음과 같습니다:
  - `basePath` (`string`)
...
`waitUntil()` 메소드는 다른 백그라운드 작업을 수행해야할 경우 함수의 실행을 연기할 수 있습니다.
...
`NextResponse` 클래스에서는 public 메소드를 사용할 수 있습니다. 사용하는 방식에 따라서, 인스턴스를 생성하고 변수에 할당하며, 그 후 다음과 같은 public 메소드에 접속할 수 있습니다:

- `cookies` - `Response`에 있는 쿠키를 포함하고 있는 [ResponseCookies](https://edge-runtime.vercel.app/packages/cookies#for-response) 인스턴스 입니다. 응답의 `Set-Cookie` 헤더를 읽거나 수정합니다. [Using cookies in Middleware](/docs/app/building-your-application/routing/middleware#using-cookies) 문서를 참고하세요.
  - `get` - 쿠키의 `name`을 입력 받은 후 `name`과 `value`로 이루어진 객체를 반환하는 메소드입니다. 입력 받은 `name`을 갖는 쿠키가 없을 경우 `undefined`를 반환합니다. 다수의 쿠키가 검색된 경우 제일 처음에 검색된 쿠키를 반환합니다.
  - `getAll` - `get`과 유사하지만 `name`과 일치하는 모든 쿠키를 배열로 반환하는 메소드입니다. `name`이 지정되지 않을 경우 검색 가능한 모든 쿠키를 반환합니다.
  - `set` - [W3C CookieStore API](https://wicg.github.io/cookie-store/#dictdef-cookielistitem) 사양에서 정의된 `CookieListItem`으로 이루어진 객체를 받는 메소드입니다.
  - `delete` - `name` 또는 다수의 이름으로 이루어진 배열을 입력받는 메소드입니다. 이름과 일치하는 쿠키를 삭제합니다. 성공적으로 삭제할 경우 `true`를 반환하며 삭제되지 않을 경우 `false`를 반환합니다.
...
- `rewrite()` - `rewrite` 세트와 함꼐 `NextResponse`를 반환합니다.
...
`redirect()`를 사용할 때, 일시적인 리다이렉트일 경우 `307` 상태 코드가 반환되고, 영구적인 리다이렉트일 경우 `308` 상태 코드가 반환되는 것을 볼 수 있습니다. 전통적으로 일시적인 리다이렉트일 경우 `302`가 반환되고 영구적인 리다이렉트일 경우 `301`이 반환됐었지만, 많은 브라우저들은 `302`를 사용할 경우 요청 메소드의 오리진과는 상관 없이 리다이렉트의 요청 메소드를 `POST` 에서 `GET` 요청으로 변경하고 있습니다.
...
- `302` - 일시적인 디라이렉트. 요청 메소드를 `POST` 에서 `GET`으로 변경합니다.
...
엣지 미들웨어에서 `process.env`를 사용하면 [환경 변수](/docs/pages/building-your-application/configuring/environment-variables)에 접근할 수 있습니다. 환경 변수들은 `next build` 과정에서 계산됩니다:
