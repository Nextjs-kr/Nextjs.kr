...
- 공식적인 `style` 프로퍼티를 지원합니다.
  - `layout` 프로퍼티를 제거하고 `style`혹은 `className`의 사용으로 대체합니다.
...
- `alt`프로퍼티는 필수 값에서 선택적 값으로 변경되었습니다. 
...
`<Image />` 컴포넌트는 필수적인 프로퍼티들과 더불어 추가적인 프로퍼티들이 존재합니다. 이번 섹션에서는 주로사용되는 이미지 컴포넌트 프로퍼티들에 대해 다루어 보고 빈번하게 사용되지 않는 프로퍼티들에 대한 내용은 [고급 프로퍼티](#고급-프로퍼티) 섹션에서 확인할 수 있습니다.
...
`loader`는 이미지의 URL을 반환하는 함수입니다. 파라메터는 다음과 같습니다.
...
```
> **알면 좋을 것**: 위의 예시에서는 `next/legacy/image`의 `src`프로퍼티가 `https://example.com/account123/`로 시작해야 함을 보장합니다. 다른 프로토콜, 호스트 명, 포트, 잘못된 경로는 `400 Bad Request`응답을 받게됩니다.
...
```
> **알면 좋을 것**:  위의 예시에서는 `next/legacy/image`의 `src`프로퍼티가 `https://img1.example.com` 혹은 `https://me.avatar.example.com`와 같은 서브도메인으로 시작해야 함을 보장합니다. 다른 프로토콜 혹은 잘못된 호스트 명은 `400 Bad Request`응답을 받게됩니다.
...
```
> **알면 좋을 것**: AVIF는 보통 20% 정도 인코딩이 오래 걸리지만, WebP에 비해 20% 더 압축 됩니다. 이 의미는 처음 이미지가 요청되었을 때는 느릴 수 있지만 이후 요청에 대해서는 캐싱되어 빠를 것이라는 것을 의미합니다.
