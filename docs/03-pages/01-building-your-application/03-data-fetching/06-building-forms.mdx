...
웹 양식에는 **클라이언트-서버** 관계가 있습니다. 처리 및 저장을 위해 웹 서버에서 처리하는 데이터를 보내는 데 사용됩니다. 폼 자체가 클라이언트이고 서버는 필요할 때 데이터를 저장, 검색 및 전송하는 데 사용할 수 있는 스토리지 매커니즘입니다.

이 가이드는 Next.js로 웹 폼 만드는 방법을 알려줍니다.

## Part 1: HTML 폼

HTML 폼은 `<form>` 태그를 이용해 작성됩니다. 텍스트 필드, 체크박스, 드롭다운 메뉴, 버튼, 라디오 버튼 등과 같은 기능에 대한 폼을 구성하기 위해 어트리뷰트와 필드의 집합이 필요합니다.
...
--`method`: 폼을 제출하는 동안 데이터 전송에 사용되는 [HTTP method](https://developer.mozilla.org/en-US/docs/Web/HTTP/Methods)를 지정합니다. 일반적으로 `GET` 또는 `POST`를 사용합니다.
...
사용자가 제공한 정보가 정확한지 여부를 확인하는 과정입니다. 또한 폼 유효성 검사는 제공된 정보가 올바른 형식인지(이메일 필드에 @가 있는지) 확인합니다. 아래에 두 가지 유형이 있습니다.

- **클라이언트 사이드**: 유효성 검사가 브라우저에서 수행됩니다.
- **서버 사이드**: 유효성 검사가 서버에서 수행됩니다.
...
이러한 유효성 검사로 사용자가 이름에 대해 빈 필드를 제출하려고 하면 바로 폼 필드에 오류가 표시됩니다. 마찬가지로, Roll 번호는 10~20자 사이의 길이로만 입력할 수 있습니다.
...
위 예시는 사용자의 이름과 roll 번호를 확인합니다. 'validateFormWithJS()' 함수는 이름 필드가 비어 있지 않도록 하며 롤 번호는 최소 세 자리 이상이어야 합니다. 유효성 검사는 제출 버튼을 누르면 수행됩니다. 주어진 값이 올바른 경우에만 다음 페이지로 리디렉션 됩니다.
...
정규 표현식을 사용한 JavaScript 유효성 검사는 `pattern` HTML 어트리뷰트를 사용합니다. 정규 표현식(RegEx)은 문자 패턴을 설명하는 개체입니다. `pattern` 어트리뷰트는 `<input>` 엘리먼트에만 적용할 수 있습니다. 이렇게 직접 규칙을 정의하여 정규 표현식(RegEx)을 이용해 입력값의 유효성을 검사할 수 있습니다. 값이 정의된 패턴과 일치하지 않으면 입력은 오류를 표시할 것입니다. 
...
비밀번호 폼 필드는 숫자(0~9), 소문자 알파벳(a~z)만 포함해야 하며 길이는 15자 이하여야 합니다. 다른 문자(#,$,& 등)는 사용할 수 없습니다. RegEx의 규칙은 `[a-z0-9]{1,15}`로 작성됩니다.
...
> 또한 위 API를 MongoDB 또는 Google 스프레드시트와 같은 데이터베이스에 연결할 수 있습니다. 이렇게 하면 제출된 폼 데이터는 이후에 사용할 수 있도록 안전하게 저장됩니다. 이 가이드에서는 데이터베이스를 사용하지 않습니다. 대신 동일한 데이터가 사용자에게 반환되어 사용 방법을 시연합니다.
...
다음은 [JSX](https://react.dev/learn/writing-markup-with-jsx)를 사용하여 작성된 [React 함수형 컴포넌트](https://react.dev/reference/react/Component)의 동일한 폼입니다.
...
아래는 사용자가 JavaScript를 비활성화하는 몇 가지 이유가 입니다.
...
JavaScript가 비활성화된 상태에서 제출 버튼을 누르면 이벤트가 트리거되어 폼 데이터를 수집하고 `action` 어트리뷰트에 정의된 대로 `POST` HTTP `method` 사용하여 폼 API 엔드포인트로 보냅니다. 폼의 `action`이 작동하는 방식으로 `/api/form` 엔드포인트로 리디렉션됩니다.

폼 데이터는 위에서 작성한 폼 핸들러 함수로 서버에 `req` 요청으로 제출됩니다. 이후 데이터를 처리하고 제출한 이름이 포함된 `res` 응답으로 JSON 문자열을 반환합니다.

> 여기에서 사용자 경험을 향상시키기 위해서 응답으로 사용자를 페이지로 리디렉션하고, 폼 제출에 대해 감사의 인사를 할 수 있습니다.
...
export default function PageWithJSbasedForm() {
  ...
  const handleSubmit = async (event) => {
    ...
    // JSON으로 서버로부터 응답 데이터를 가져옵니다.  
    // 서버가 제출된 이름을 반환하면 폼이 작동한다는 의미입니다.
