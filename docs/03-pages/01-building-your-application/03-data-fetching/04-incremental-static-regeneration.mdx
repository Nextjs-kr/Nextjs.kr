...
> **알아두면 좋은 사항**: [`edge` runtime](/docs/pages/api-reference/edge)은 현재 ISR과 호환되지 않지만, `cache-control` 헤더를 수동으로 설정하여 `stale-while-revalidate`를 활용할 수 있습니다.
...
> **알아두면 좋은 사항**: 주요 데이터 공급 업체에서 기본적으로 캐싱이 활성화되어 있는지 확인하세요. 새로운 데이터를 가져와 ISR 캐시를 업데이트할 수 없게 될 수 있으므로 비활성화(예: `useCdn: false` 사용)해야 할 수도 있습니다. 캐싱은 요청되는 엔드포인트에서 `Cache-Control` 헤더를 반환할 때 CDN에서 발생할 수 있습니다.
...
> **알아두면 좋은 사항**: [미들웨어](/docs/pages/building-your-application/routing/middleware)는 On-Demand ISR 요청에 대해 실행되지 않습니다. 대신, 다시 유효화하려는 _정확한_ 경로에서 `revalidate()`를 호출하세요. 예를 들어, `pages/blog/[slug].js`와 `/post-1` -> `/blog/post-1`로 재작성하는 경우, `res.revalidate('/blog/post-1')`을 호출해야 합니다.

### 요청에 따른 재검증을 사용하는 방법

먼저, Next.js 앱에서만 알고 있는 비밀 토큰을 생성하세요. 이 비밀 토큰은 재검증 API 라우트에 무단 액세스를 방지하는 데 사용됩니다. 다음 URL 구조를 사용하여 라우트에 액세스할 수 있습니다(수동으로 또는 webhook을 사용).
...
> **알아두면 좋은 사항**: 공유 마운트가 구성된 방식에 따라 동시에 캐시를 업데이트하려는 여러 개의 pod 간의 경합 상태를 고려해야 할 수도 있습니다.
