...
사용자 404페이지를 만들기 위해서는 `pages/500.js` 파일을 만들어야합니다. 이 파일은 빌드 시 정적으로 생성됩니다.
...
- `_app` 처럼 `_error`는 예약 경로입니다. `_error`는 커스텀 레이아웃을 정의하고 에러 페이지 행동을 정의하는데 사용됩니다. `/_error` 는 [routing](/docs/pages/building-your-application/routing)를 통해 직접 접근하거나 [custom server](/docs/pages/building-your-application/configuring/custom-server) 안에서 렌더링이 일어날때 404를 렌더링합니다.
