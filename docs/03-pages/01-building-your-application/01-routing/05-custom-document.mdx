...
- `_document`에서 사용되는 `<Head />` 컴포넌트는 [`next/head`](/docs/03-pages/02-api-reference/01-components/head.mdx)와 다릅니다. 여기에서 사용된 `<Head />` 컴포넌트는 모든 페이지에 공통인 `<head>` 코드로써만 사용되어야 합니다. 다른 경우에는 각 페이지나 컴포넌트에서 [`next/head`](/docs/pages/api-reference/components/head)를 사용하는 것이 좋습니다.
