...
더 나은 타입 안정성을 위해, `req` 및 `res` 객체를 확장하는 것을 권장하지 않습니다. 대신, 이들과 함께 작동하는 함수를 사용하세요.
...
만약 이러한 객체를 확장하는 것을 피할 수 없다면, 추가적인 속성을 포함하기 위해 타입을 만들어야 합니다.
...
- `res.redirect([status,] path)` - 지정된 경로 또는 URL로 리다이렉트합니다. `status`는 유효한 [HTTP 상태 코드](https://ko.wikipedia.org/wiki/HTTP_%EC%83%81%ED%83%9C_%EC%BD%94%EB%93%9C)여야 합니다.
  만약 지정하지 않는다면 `status`의 기본값은 "307" 즉, "임시 리다이렉트(Temporary redirect)"입니다.
