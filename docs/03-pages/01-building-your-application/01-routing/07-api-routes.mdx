...
`pages/api` 폴더 내의 모든 파일은 `/api/*`에 대응되며 `page`가 아닌 API 엔드포인트로 취급됩니다. 이들은 서버 측 번들이며 클라이언트 측 번들 크기를 증가시키지 않습니다.
...
- `req`: [http.IncomingMessage](https://nodejs.org/api/http.html#class-httpincomingmessage) 인스턴스와 [미리 만들어진 미들웨어](/docs/pages/api-reference/functions/next-server)들
- `res`: [http.ServerResponse](https://nodejs.org/api/http.html#class-httpserverresponse) 인스턴스와 [헬퍼 함수](/docs/pages/api-reference/functions/next-server)들
...
- API 라우트는 [CORS 헤더를 지정하지 않습니다.](https://developer.mozilla.org/ko/docs/Web/HTTP/CORS) 이는 기본적으로 **동일 출처 전용**임을 의미합니다.
...
자동 `bodyParsing`을 비활성화하는 사용 사례는 **웹훅** 요청의 원시 본문을 확인하려는 경우입니다. 예를 들어 [GitHub으로부터의 웹훅](https://docs.github.com/ko/webhooks-and-events/webhooks/securing-your-webhooks) 요청을 받을 수 있습니다.
...
`responseLimit`은 바이트 수 또는 `bytes`가 지원하는 어떤 문자열 형식도 받을 수 있습니다. 예를 들어 `1000`, `500kb` 또는 `3mb` 등이 가능합니다.
...
더 나은 타입 안정성을 위해, `req` 및 `res` 객체를 확장하는 것은 권장되지 않습니다. 대신, 이들과 함께 작동하는 함수를 사용하세요.
...
만약 이러한 객체들이 확장되는 것을 피할 수 없다면, 추가적인 속성을 포함하기 위해 타입을 만들어야 합니다.
...
- `res.status(code)` - 상태 코드를 설정하는 함수입니다. `code`는 유효한 [HTTP 상태 코드](https://ko.wikipedia.org/wiki/HTTP_%EC%83%81%ED%83%9C_%EC%BD%94%EB%93%9C)여야 합니다.
...
- `res.redirect([status,] path)` - 지정된 경로 또는 URL로 리다이렉트합니다. `status`는 유효한 [HTTP 상태 코드](https://ko.wikipedia.org/wiki/HTTP_%EC%83%81%ED%83%9C_%EC%BD%94%EB%93%9C)여야 합니다.
  만약 지정되지 않는다면, `status`의 기본값은 "307" 즉, "임시 리다이렉트(Temporary redirect)"입니다.
...
다음 예시는 폼이 성공적으로 제출되면 클라이언트를 `/` 경로로 리다이렉트합니다.
...
API 라우트는 괄호 안에 세 개의 점(`...`)을 추가함으로써 모든 경로를 잡아내도록 확장될 수 있습니다. 예를 들어
...
매칭된 매개변수는 쿼리 매개변수(`slug`가 예시)로 페이지에 전달되며, 이 매개변수는 항상 배열 형태입니다. 따라서, `/api/post/a` 경로는 다음과 같은 `query` 객체를 가질 것입니다.
...
에지 API 라우트를 사용하면 Next.js로 고성능 API를 만들 수 있습니다. [에지 런타임](/docs/pages/api-reference/edge)을 사용하기 때문에, 종종 Node.js 기반 API 라우트보다 빠릅니다. 이러한 성능 개선은 기본 Node.js API에 접근할 수 없는 [제약사항](/docs/pages/api-reference/edge#unsupported-apis)이 따르지만, 대신 에지 API 라우트는 표준 웹 API를 기반으로 합니다.

`pages/api` 폴더 내의 모든 파일은 `api/*`로 대응되며, 페이지가 아닌 API 엔드포인트로 처리됩니다. 이들은 서버 측 전용 번들이며 클라이언트 측 번들 크기는 증가하지 않습니다.
...
배포할 때 에지 함수를 특정 지역으로 제한하고 싶을 수 있습니다. 이렇게 하면 데이터 소스와 가까운 곳에 서버를 위치시켜 응답 시간을 줄일 수 있습니다. 이는 아래와 같이 구현할 수 있습니다.
...
에지 API 라우트는 [에지 런타임](/docs/pages/api-reference/edge)을 사용하며, API 라우트는 [Node.js 런타임](/docs/app/building-your-application/rendering/edge-and-nodejs-runtimes)을 사용합니다.

에지 API 라우트는 서버로부터 [응답을 스트림](/docs/pages/api-reference/edge#stream-apis)할 수 있으며, 캐시된 파일들(예: HTML, CSS, JavaScript)에 접근한 *이후*에 실행됩니다. 서버 측 스트리밍은 더 빠른 [Time To First Byte (TTFB)](https://web.dev/ttfb/)로 성능을 향상할 수 있습니다.

> **참고**: `getServerSideProps`와 함께 [에지 런타임](/docs/pages/api-reference/edge)을 사용하면 응답 객체에 접근할 수 없습니다. `res`에 접근이 필요하다면 `runtime: 'nodejs'`를 설정하여 [Node.js 런타임](/docs/app/building-your-application/rendering/edge-and-nodejs-runtimes)을 사용해야 합니다.

에지 런타임에서 [지원하는 API](/docs/pages/api-reference/edge)와 [지원하지 않는 API](/docs/pages/api-reference/edge#unsupported-apis)를 확인해 보세요.
