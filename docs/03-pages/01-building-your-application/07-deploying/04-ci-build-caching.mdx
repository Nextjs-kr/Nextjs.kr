...
> 만약 CI가 `.next/cache`를 빌드 사이에서 유지하지 않도록 설정되어 있다면, [No Cache Detected](/docs/messages/no-cache) 오류가 발생할 수 있습니다.
...
만약 `save_cache` 키가 없다면, CircleCI의 [빌드 캐싱 설정 문서](https://circleci.com/docs/2.0/caching/)를 따라 진행합니다.
