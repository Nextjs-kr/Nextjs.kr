...
Next.js 애플리케이션의 개발 단계에서 런타임 오류가 발생하면, **오버레이**를 만나게 될 것입니다. 이는 웹페이지를 덮는 모달입니다. 오직 `next dev`를 통해 개발 서버가 실행될 때 (`pnpm dev`, `npm run dev` 또는 `yarn dev` 사용 시)에만 보여지며, 프로덕션(실제 서비스)에서는 보여지지 않습니다. 오류를 수정하면 오버레이는 자동으로 사라집니다.
