...
> **경고**: PostCSS config 파일을 정의하면 Next.js는 [기본 동작](#default-behavior)을 **완전히 비활성화**합니다.
> [Autoprefixer](https://github.com/postcss/autoprefixer)를 포함하여 컴파일해야 하는 모든 기능을 수동으로 구성해야합니다.
> 또한 커스텀한 구성에 포함된 모든 플로그인을 수동으로 설치해야 합니다. `npm install postcss-flexbugs-fixes postcss-preset-env`.
...
> **알아두면 좋은 점**: Next.js는 파일이름을 `.postcssrc.json`로 지정하거나, 혹은 `package.json`파일에서 `postcss` 키를 읽어오는 것을 허용합니다.
...
> **알아두면 좋은 점**: Next.js는 파일이름을 `.postcssrc.js`로 지정하는 것을 허용합니다.

PostCSS 플러그인을 불러올 때 **`require()`을 사용하지 마세요.** 플러그인은 문자열로 받아와야 합니다.

> **알아두면 좋은 점**: 같은 프로젝트에서 non-Next.js 도구를 지원하려면 상호운용되는 객체 기반 형식을 사용해야합니다:
>
> ```js
> module.exports = {
>   plugins: {
