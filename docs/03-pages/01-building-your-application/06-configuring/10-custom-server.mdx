...
기본적으로, Next.js에는 `next start`로 실행시킬 수 있는 서버를 포함하고 있습니다. 백엔드 서버가 있다면, Next.js와 함께 사용할 수 있습니다.(이것은 사용자 정의 서버가 아닙니다). Next.js 사용사 정의 서버는 사용자 정의 서버 패턴을 사용하기 위해 100% 프로그래밍 방식으로 서버를 실행할 수 있도록 해줍니다. 대부분의 경우 이 기능이 필요하지 않습니다. - 그러나 완전한 사용자 지정이 가능합니다.
...
> `useFileSystemPublicRoutes`는 SSR에서 파일 이름 경로를 비활성화 합니다; 클라이언트 측의 라우팅은 여전히 비활성화된 경로들에 접근할 수 있습니다. 이 옵션을 사용할 때는 원하지 않는 경로로 이동하지 않게 프로그래밍적으로 방지해야 합니다.
