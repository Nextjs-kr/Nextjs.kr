...
이 API 라우트에서, 응답 객체에 대해 `setPreviewData`를 호출해야 합니다. `setPreviewData`의 매개변수는 객체여야만 하며, `getStaticProps`에 의해 사용될 수 있습니다(이에 대해 후술합니다). 이제부터 우리는 `{}`를 사용할 겁니다.
...
실전에서는 headless CMS로부터 _안전하게_ 이 API 라우트를 호출하고 싶을 겁니다. 구체적인 절차는 사용하고 있는 CMS에 따라 다르겠지만, 일반적으로 수행할 수 있는 몇몇 절차가 있습니다.
...
이제 다 됐습니다! headless CMS로부터 혹은 수동으로 미리보기 API 라우트에 접근하면(`secret`과 `slug`를 통해), 미리보기 내용을 볼 수 있을 겁니다. 그리고 초안을 게시 없이 업데이트 하면 초안을 미리보기 할 수 있을 겁니다.
...
그 다음, API 라우트를 호출하기 위해 `/api/clear-preview-mode-cookies`로 요청을 보내세요. 이 라우트를 [`next/link`](/docs/pages/api-reference/components/link)를 사용해 호출한다면, 링크 프리페치 동안 `clearPreviewData`의 호출을 막기 위해 `prefetch={false}`를 넘겨주어야 합니다.
...
미리보기 모드는 `getServerSideProps`에서도 작동합니다. 또한 `preview`와 `previewData`를 포함하는 `context` 객체에서도 사용 가능합니다.
...
`next build`가 완료될 때 바이패스 쿠키 값과 `previewData`를 암호화하는 데 사용되는 개인 키 모두 변경됩니다.
