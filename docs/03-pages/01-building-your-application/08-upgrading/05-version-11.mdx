...
이제 Next.js는 Webpack 5를 기본으로 사용합니다. 별 다른 설정을 하지 않는다면 애플리케이션은 자동으로 webpack 5를 사용할 것입니다. webpack 설정을 변경해야 한다면 [Next.js webpack 5 documentation](/docs/messages/webpack5)을 참고하세요.

### `distDir`는 이제 자동으로 제거됩니다.

빌드 결과물이 생성되는 디렉터리 (기본 설정은 `.next`입니다) 는 이제 Next.js 캐시를 제외하고 자동으로 제거됩니다. 더 자세한 정보는 [the cleaning `distDir` RFC](https://github.com/vercel/next.js/discussions/6009)를 참고하세요.
...
Next.js 11 버전에서 요구하는 React의 최소 버전이 17.0.2이기 때문에, 대부분의 애플리케이션은 이미 최신 버전의 React를 사용합니다.
